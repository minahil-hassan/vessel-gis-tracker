# # src/api/endpoints/dashboard.py
...
# from src.utils.constants import SHIP_TYPE_MAP, VESSEL_TYPE_GROUPS, LIVERPOOL_BBOX
...
#     # Calculate average speed over ground (sog)
#     avg_speed = round(sum([v.get("sog", 0) or 0 for v in vessel_docs]) / total_vessels, 2) if total_vessels else 0

#     # Find the most recent timestamp in the data
#     valid_timestamps = [
#         v.get("timestamp_utc") for v in vessel_docs
...
#     # Lookup vessel types and destinations from vessel_details collection
#     mmsi_list = [v.get("mmsi") for v in vessel_docs]
#     details_lookup = {
#         doc["mmsi"]: doc for doc in db["vessel_details"].find(
#             {"mmsi": {"$in": mmsi_list}}, {"_id": 0, "mmsi": 1, "Type": 1, "Destination": 1}
#         )
#     }

#     # Count vessels by type and by destination
#     type_counts = {}
#     destination_counts = {}

#     for mmsi in mmsi_list:
#         detail = details_lookup.get(mmsi, {})
#         tcode = detail.get("Type")
#         tlabel = SHIP_TYPE_MAP.get(tcode, f"Type {tcode}" if tcode else "Unknown")
#         type_counts[tlabel] = type_counts.get(tlabel, 0) + 1

#         dest = (detail.get("Destination") or "").strip()
#         if dest:
#             destination_counts[dest] = destination_counts.get(dest, 0) + 1

#     # Group vessel types into broader categories expected by frontend
#     grouped_subtypes = {}
#     for label, count in type_counts.items():
#         found = False
#         for group, subtypes in VESSEL_TYPE_GROUPS.items():
#             if label in subtypes:
#                 grouped_subtypes.setdefault(group, []).append((label, count))
#                 found = True
#                 break
#         if not found:
#             grouped_subtypes.setdefault("Other / Unknown", []).append((label, count))

#     # Format grouped data into frontend-friendly nested dictionary
#     grouped_vessel_types = {
#         group: {subtype: count for subtype, count in subtypes}
#         for group, subtypes in grouped_subtypes.items()
#     }

#     # Top 5 destinations by vessel count
#     top_destinations = sorted(destination_counts.items(), key=lambda x: x[1], reverse=True)[:5]
#     top_destinations = [d[0] for d in top_destinations]

#     # Query vessels seen in Liverpool in the last 3 days
#     since = datetime.now(timezone.utc) - timedelta(days=days)
#     liverpool_query = {
#         "timestamp_utc": {"$gte": since},
#         "coordinates": {"$geoWithin": {"$geometry": LIVERPOOL_BBOX}}
#     }
#     liverpool_positions = list(db["vessel_position"].find(liverpool_query, {"_id": 0}))
#     unique_liverpool_mmsi = {v.get("mmsi") for v in liverpool_positions if v.get("mmsi")}
#     liverpool_count = len(unique_liverpool_mmsi)

#     avg_liverpool_speed = round(
#         sum([v.get("sog", 0) or 0 for v in liverpool_positions]) / len(liverpool_positions), 2
#     ) if liverpool_positions else 0

#     # Return structured dashboard response
#     return JSONResponse({
#         "uk": {
#             "total_vessels": total_vessels,
#             "avg_speed": avg_speed,
#             "last_update": last_update,
#             "grouped_vessel_types": grouped_vessel_types,
#             "top_destinations": top_destinations
#         },
#         "liverpool": {
#             "vessel_count_last_3d": liverpool_count,
